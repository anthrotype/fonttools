...
GLIF_FORMAT_2_1 = FormatVersion(2, 1)
...
class GlyphSet(_UFOBaseIO):
	...
	def readGlyph(self, glyphName, glyphObject=None, pointPen=None, validate=None):
		...
		elif self.ufoFormatVersionTuple >= UFO_FORMAT_3_1:
			formatVersions.add(GLIF_FORMAT_2_1)
	...
	def writeGlyph(self, glyphName, glyphObject=None, drawPointsFunc=None, formatVersion=None, validate=None):
		...
		if formatVersion is None:
			if self.ufoFormatVersionTuple >= UFO_FORMAT_3_1:
				formatVersion = GLIF_FORMAT_2_1
			elif self.ufoFormatVersionTuple == UFO_FORMAT_3_0:
		...
		if (formatVersion.major == 2 and self.ufoFormatVersionTuple.major < 3) or (
			formatVersion == (2, 1) and self.ufoFormatVersionTuple < (3, 1)
		):
			raise GlifLibError(
...
_READ_GLYPH_FROM_TREE_FUNCS = {
	...
	GLIF_FORMAT_2_1: partial(_readGlyphFromTreeFormat2, formatMinor=1)
}
